...
setup_protobuf  = '%s ; from google.protobuf.struct_pb2 import Struct; s = Struct(); s.update(d); src = s.SerializeToString()' % message
...
tests = [
    ...
    ('protobuf', 'from google.protobuf.struct_pb2 import Struct; %s' % setup_protobuf, 's.SerializeToString()', 's.ParseFromString(src)'),
    ('yaml', 'import yaml; %s' % setup_yaml, 'yaml.dump(d)', 'yaml.load(src)'),
